...
class EmailService:
    ...
    3. Intelligent email search using either RAG or contextual search
    4. Formatting results for mailbox display
    
    Search methods:
    - RAG search: Uses local embeddings and vector storage (use_rag=True)
    - Contextual search: Sends emails directly to Albert API (use_rag=False)
    """
    ...
    def __init__(self, use_rag: bool = False):
        ...
        Args:
            use_rag: Boolean flag to decide between RAG search (True) or contextual search (False)
        """
        self.logger = logger
        self.use_rag = use_rag
        self.logger.info(f"EmailService initialized with use_rag={use_rag}")
        if use_rag:
            self.logger.info("Using RAG-based intelligent email search")
        else:
            self.logger.info("Using contextual search with Albert API")
    ...
    def chatbot_intelligent_email_search(
        ...
        """
        self.logger.info(f"Starting intelligent email search - method: {'RAG' if self.use_rag else 'contextual'}")
        
        if self.use_rag:
            # Use RAG-based search (existing implementation)
            return self._chatbot_rag_email_search(user_id, user_query, api_client, max_results)
        else:
...
email_service = EmailService(use_rag=False)

# Convenience functions to create services with specific configurations
def create_rag_email_service():
    """Create an email service that uses RAG for intelligent search."""
    return EmailService(use_rag=True)

def create_contextual_email_service():
    """Create an email service that uses contextual search with Albert API."""
    return EmailService(use_rag=False)

# Alternative service instances for different use cases
rag_email_service = EmailService(use_rag=True)
contextual_email_service = EmailService(use_rag=False)
