...
def list_labels_of_user(factory, user, mailbox_id):
    ...
    print(f"FABIAN USER ID: {user.id}")
    print(f"FABIAN MAILBOX ID: {mailbox_id}")
    print(f"ALL LABELS FABIAN: {list(response.values())}")
...
def assign_label_to_thread(thread: Thread, mailbox_id):
    ...
    print(f"FABIAN RECIPIENT EMAIL: {recipient_email} {type(recipient_email)}")
    user = User.objects.get(email=recipient_email)

    labels = list_labels_of_user(factory, user, mailbox_id)
    print("FABIAN LABELS:", labels)
    ...
    for label_name in best_labels:
        ...
        print(f"FABIAN RESPONSE ADD THREAD TO LABEL: {response.data}")
