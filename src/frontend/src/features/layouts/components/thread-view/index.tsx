...
export const ThreadView = () => {
  ...
  const { selectedMailbox, selectedThread, messages, queryStates } =
    useMailboxContext();
  ...
  const messagesWithDraftChildren = useMemo(() => {
    ...
    const rootMessages: MessageWithDraftChild[] = messages.results.filter(
      (m) => !m.is_draft || !m.parent_id
    );
    const draftChildren = messages.results.filter(
      (m) => m.is_draft && m.parent_id
    );
  ...
  const filteredMessages = useMemo(() => {
    ...
    return messagesWithDraftChildren.filter(
      (m) => m.is_trashed === isTrashView
    );
  }, [messagesWithDraftChildren, isTrashView, showTrashedMessages]);

  const latestMessage = messagesWithDraftChildren
    .filter((m) => m.is_trashed === isTrashView)
    .reduce((acc, message) => {
  ...
  const unreadMessageIds =
    messages?.results?.filter((m) => !m.read_at).map((m) => m.id) || [];
  const trashedMessageIds =
    messages?.results?.filter((m) => m.is_trashed).map((m) => m.id) || [];
          ...
          return (
            <ThreadMessage
              ...
              ref={
                isUnread
                  ? (el) => {
                      unreadRefs.current[message.id] = el;
                    }
                  : undefined
              }
        ...
        {trashedMessageIds.length > 0 && !showTrashedMessages && (
          ...
          >
            <div className="thread-view__trashed-banner__content">
              <p>
                {t("thread-view.trashed-banner.message", {
